...
export async function handleGetCollectionByUsername(username: string) {
  const response = await fetch(
    `${API_BASE_URL}/profile/${username}/collections/`,
    {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`
...
export async function handleSaveImageToCollection(
  collectionId: string,
  imageId: number[]
) {
  ...
  const combinedImageIds = Array.from(
    new Set([...currentImageIds, ...imageId])
  );
...
export async function loadImagesFromCollection(
  username: string,
  collectionId: string
) {
  // Xác định URL dựa trên việc có username hay không
...
export async function handleRemoveImageFromCollection(
  collectionId: string,
  imageId: number
) {
  const response = await fetch(`${API_BASE_URL}/collections/${collectionId}/`, {
